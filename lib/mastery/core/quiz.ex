defmodule Mastery.Core.Quiz do
  ...
  defp reset_record(%{current_question: _} = quiz) do
    new_record = Map.delete(quiz.record, template(quiz).name)
    Map.put(quiz, :record, new_record)
  end

  defp inc_record(%{current_question: _} = quiz) do
  ...
  defp select_a_random_question(quiz) do
    # Select a random template and from it, select a random question
  ...
  defp reset_template_cycle(%{templates: templates, used: used} = quiz) when map_size(templates) == 0 do
    %__MODULE__{ quiz | templates: Enum.group_by(used, fn template -> template.category end), used: [] }
  end
